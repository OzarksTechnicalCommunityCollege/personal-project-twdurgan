...
def post_view(request, id):
    ...
    form = CommentForm()
    # Similar posts: Ordering is done as in the book, but in the future the secondary ordering should be via a user score system.
    post_tags_ids = post.tags.values_list('id', flat=True)
    similar_posts = Post.viewable.filter(tags__in=post_tags_ids).exclude(id=post.id)
